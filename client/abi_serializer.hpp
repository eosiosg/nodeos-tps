 ...
 */
        struct abi_serializer {
            ...
            fc::optional<string>  get_error_message( uint64_t error_code )const;
        ...
        namespace impl {
            ...
            struct abi_to_variant {
                 ...
                 */
                template<typename M, typename Resolver, require_abi_t<M> = 1>
                static void add( mutable_variant_object &mvo, const char* name, const std::vector<M>& v, Resolver resolver, abi_traverse_context& ctx )
                {
                    auto h = ctx.enter_scope();
                    std::vector<fc::variant> array;
            ...
            struct abi_from_variant {
                 ...
                 */
                template<typename M, typename Resolver, require_abi_t<M> = 1>
                static void extract( const fc::variant& v, std::vector<M>& o, Resolver resolver, abi_traverse_context& ctx )
                {
                    auto h = ctx.enter_scope();
        ...
        template<typename T, typename Resolver>
        void abi_serializer::to_variant( const T& o, fc::variant& vo, Resolver resolver, const fc::microseconds& max_serialization_time ) try {
            mutable_variant_object mvo;
        ...
        template<typename T, typename Resolver>
        void abi_serializer::from_variant( const fc::variant& v, T& o, Resolver resolver, const fc::microseconds& max_serialization_time ) try {
            impl::abi_traverse_context ctx(max_serialization_time);
