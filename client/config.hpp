...
namespace eosio { namespace chain { namespace config {
            ...
            const static uint64_t system_account_name    = eosio::chain::string_to_name("eosio");
            const static uint64_t null_account_name      = eosio::chain::string_to_name("eosio.null");
            const static uint64_t producers_account_name = eosio::chain::string_to_name("eosio.prods");

// Active permission of producers account requires greater than 2/3 of the producers to authorize
            const static uint64_t majority_producers_permission_name = eosio::chain::string_to_name("prod.major"); // greater than 1/2 of producers needed to authorize
            const static uint64_t minority_producers_permission_name = eosio::chain::string_to_name("prod.minor");; // greater than 1/3 of producers needed to authorize0

            const static uint64_t eosio_auth_scope       = eosio::chain::string_to_name("eosio.auth");
            const static uint64_t eosio_all_scope        = eosio::chain::string_to_name("eosio.all");

            const static uint64_t active_name = eosio::chain::string_to_name("active");
            const static uint64_t owner_name  = eosio::chain::string_to_name("owner");
            const static uint64_t eosio_any_name = eosio::chain::string_to_name("eosio.any");
            const static uint64_t eosio_code_name = eosio::chain::string_to_name("eosio.code");
