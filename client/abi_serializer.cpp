        ...
        auto pack_unpack() {
            return std::make_pair<abi_serializer::unpack_function, abi_serializer::pack_function>(
                    []( fc::datastream<const char*>& stream, bool is_array, bool is_optional) -> fc::variant  {
                        if( is_array )
                            return variant_from_stream<std::vector<T>>(stream);
                        else if ( is_optional )
                        return variant_from_stream<fc::optional<T>>(stream);
                        return variant_from_stream<T>(stream);
                    },
                    []( const fc::variant& var, fc::datastream<char*>& ds, bool is_array, bool is_optional ){
                        if( is_array )
                            fc::raw::pack( ds, var.as<std::vector<T>>() );
                        else if ( is_optional )
...
//            built_in_types.emplace("extended_asset",            pack_unpack<extended_asset>());
        ...
        void abi_serializer::set_abi(const abi_def& abi, const fc::microseconds& max_serialization_time) {
            ...
            for( const auto& td : abi.types ) {
                auto is_type = _is_type(td.type, ctx);
                EOS_ASSERT(is_type, invalid_type_inside_abi, "invalid type ${type}", ("type",td.type));
        ...
        void abi_serializer::validate( impl::abi_traverse_context& ctx )const {
            for( const auto& t : typedefs ) { try {
                    std::vector<type_name> types_seen{t.first, t.second};
            ...
            for( const auto& s : structs ) { try {
                    if( s.second.base != type_name() ) {
                        ...
                        std::vector<type_name> types_seen{current.name};
                        while( current.base != type_name() ) {
                            ctx.check_deadline();
                            const auto& base = get_struct(current.base); //<-- force struct to inherit from another struct
                            auto flag = find(types_seen.begin(), types_seen.end(), base.name) == types_seen.end();
                            EOS_ASSERT( flag, abi_circular_def_exception, "Circular reference in struct ${type}", ("type",s.second.name) );
                    ...
                    for( const auto& field : s.second.fields ) { try {
                            ctx.check_deadline();
                            auto rb = _remove_bin_extension(field.type);
                            auto isType = _is_type(rb, ctx);
                            EOS_ASSERT(isType, invalid_type_inside_abi, "${type}", ("type",field.type) );
                ...
                impl::binary_to_variant_context& ctx )const
        {
            ...
            if ( is_array(rtype) ) {
                ...
                if( v_itr != variants.end() ) {
                    ...
                    return std::vector<fc::variant>{v_itr->second.types[select], _binary_to_variant(v_itr->second.types[select], stream, ctx)};
                }
            }
        ...
        { try {
                ...
                if( btype != built_in_types.end() ) {
                    ...
                    ctx.hint_array_type_if_in_array();
                    std::vector<fc::variant> vars = var.get_array();
        ...
        fc::optional<string> abi_serializer::get_error_message( uint64_t error_code )const {
            auto itr = error_messages.find( error_code );
            if( itr == error_messages.end() )
                return fc::optional<string>();
